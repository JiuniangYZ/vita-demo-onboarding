...
export function ResultScreen({ config }: ResultScreenProps) {
  ...
  // 个性化文本
  const title = config.usePersonalization ? personalizeText(config.title, userData) : config.title
  const subtitle = config.usePersonalization ? personalizeText(config.subtitle, userData) : config.subtitle
      ...
      <div className="flex-1 px-5 pb-4 overflow-y-auto scrollbar-hide">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <p className="text-[24px] font-bold mt-1" style={{ color: '#2B2735' }}>
              <AnimatedNumber value={displayResults.tdee || 0} />
            </p>
          ...
          >
            ...
            <p className="text-[24px] font-bold mt-1" style={{ color: '#2B2735' }}>
              {isLosing ? '-' : isGaining ? '+' : ''}{displayResults.weeklyLoss || 0}kg
            </p>
