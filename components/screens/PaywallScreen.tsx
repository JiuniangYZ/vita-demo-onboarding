...
export function PaywallScreen({ config }: PaywallScreenProps) {
  ...
  const rawTitle = config.usePersonalization ? personalizeText(config.title, userData) : config.title
  const rawSubtitle = config.usePersonalization ? personalizeText(config.subtitle, userData) : config.subtitle
  
  const isDiscountTitle = rawTitle?.includes('OFF') || rawTitle?.includes('%')
  const title = isDiscountTitle && !hasDiscount 
    ? personalizeText('{name}, start your transformation', userData)
    : rawTitle
